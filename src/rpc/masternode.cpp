...
static UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print 5G address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               C5GAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                C5GAddress address(mn.pubKeyCollateralAddress.GetID());
...
//            return mnodeman.CountEnabled(MIN_PRIVATESEND_PEER_PROTO_VERSION);
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.vin.prevout.ToString()));
        obj.push_back(Pair("payee",         C5GAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", C5GAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
    ...
    if (strCommand == "decode")
    {
        ...
        for(CMasternodeBroadcast& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", C5GAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", C5GAddress(mnb.pubKeyMasternode.GetID()).ToString()));
